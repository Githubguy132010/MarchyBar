...
function fixture(t, { failBrightness = false, failPersistence = false, wakeDuringOpen = false, runtime = false, persisted = false, profile = 't2' } = {}) {
  ...
  const kernel = profile === 't2' ? '7.2.5-arch1-T2' : 'fixture-kernel';
  t.mock.method(fs, 'readFileSync', (file, ...args) => {
    ...
    return file === '/sys/devices/virtual/dmi/id/product_name' ? (profile === 't2' ? 'MacBookPro15,1' : '') : file === '/proc/sys/kernel/osrelease' ? kernel : read(file, ...args);
  });
  t.mock.method(fs, 'existsSync', file => file === '/run/marchybar/device.sock' || file === (profile === 'asahi' ? '/sys/module/adpdrm' : '/sys/module/appletbdrm') ? true : file === 'fixture:control' || String(file).startsWith(`/usr/lib/modules/${kernel}/`) ? false : exists(file));
