...
function fixture({ model = '', compatible = null, paths = [], entries = [] } = {}) {
  const files = {
...
for (const model of MODELS) {
  test(`DMI ${model} retains the T2 profile and driver check`, () => {
    const result = fixture({ model: model + '\n', paths: ['/sys/module/appletbdrm', '/run/marchybar/device.sock'] });
    assert.deepEqual(result, { model, profile: 't2', experimental: false, kernel: 'fixture-kernel', supported: true, driver: true, broker: true, status: 'available' });
  });
}
...
for (const [compatible, model] of [['apple,j293', 'MacBookPro17,1'], ['apple,j493', 'Mac14,7']]) {
  test(`${compatible} requires a recognized first entry and takes precedence over T2 DMI`, () => {
    for (const value of [`${compatible}\0apple,t8103\0apple,arm-platform\0`, `${compatible}\0`]) {
      ...
      assert.deepEqual(result, { model, profile: 'asahi', experimental: true, kernel: 'fixture-kernel', supported: true, driver: false, broker: false, status: 'setup-required' });
    }
  });
}
