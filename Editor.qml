...
Item {
  ...
  readonly property bool dirty: draft && JSON.stringify(draft) !== savedDraft
  readonly property bool rulesDirty: JSON.stringify(ruleDraft) !== rulesBaseline
  ...
  function guard(action) {
    if (dirty || rulesDirty) confirm("Discard unsaved changes?", "Your saved presets and app rules will stay as they are.", function() { root.draft = null; root.savedDraft = ""; root.loadRules(); action() })
    else action()
  }
  function confirm(title, text, action) { confirmTitle = title; confirmText = text; confirmAction = action }
  function prompt(title, value, action) { promptTitle = title; promptValue = value; promptAction = action; promptVisible = true; Qt.callLater(() => promptField.forceActiveFocus()) }
  function load(id) {
  ...
  function changeWidget(key, value) { mutate(d => { d.pages[pageIndex].widgets[widgetIndex][key] = value }) }
  function moveWidget(from, to) {
    if (!page || to < 0 || to >= page.widgets.length || from === to) return
    mutate(d => { var w = d.pages[pageIndex].widgets.splice(from,1)[0]; d.pages[pageIndex].widgets.splice(to,0,w) }); widgetIndex = to
  }
  function undo() { if (!history.length) return; var h = history.slice(); var f = future.slice(); f.push(copy(draft)); draft = h.pop(); history = h; future = f; pageIndex = Math.min(pageIndex,draft.pages.length-1); widgetIndex = 0; previewTimer.restart() }
  function redo() { if (!future.length) return; var f = future.slice(); var h = history.slice(); h.push(copy(draft)); draft = f.pop(); future = f; history = h; pageIndex = Math.min(pageIndex,draft.pages.length-1); widgetIndex = 0; previewTimer.restart() }
  function save() {
    call("preset.save", {preset:draft}, function(ok, data) { if (ok) { root.savedDraft = JSON.stringify(root.draft); root.editRevision = root.model.revision; root.notice = "Preset saved" } }, editRevision)
  }
  function preview() {
    if (!opened || !draft || !page) return
    call("preview", {preset:draft,page:page.id,width:previewWidth}, function(ok,data) { if (ok) { root.boxes = data.boxes; root.previewError = "" } else root.previewError = data })
  }
  ...
  Connections {
    ...
    function onSnapshotChanged() {
      ...
      if (!root.rulesDirty) root.loadRules()
  ...
  FloatingWindow {
    ...
    Shortcut { sequence: "Ctrl+S"; enabled: root.opened && root.tab===0 && root.dirty; onActivated: root.save() }
    Shortcut { sequence: "Ctrl+Z"; enabled: root.opened && root.tab===0; onActivated: root.undo() }
    Shortcut { sequence: "Ctrl+Shift+Z"; enabled: root.opened && root.tab===0; onActivated: root.redo() }
      ...
      StackLayout {
        ...
        RowLayout {
          ...
          ColumnLayout {
            ...
            RowLayout {
              Ui.Dropdown { Layout.fillWidth:true; options:root.pageOptions; value:root.page?root.page.id:""; onChanged:value=>{root.pageIndex=root.draft.pages.findIndex(p=>p.id===value);root.widgetIndex=0;previewTimer.restart()} }
              Button { text:"+ Page"; onClicked:root.prompt("New page","New page",name=>{root.mutate(d=>d.pages.push({id:"page-"+Date.now(),name:name,widgets:[{id:"clock",type:"clock",weight:1}]}));root.pageIndex=root.draft.pages.length-1;root.widgetIndex=0}) }
              Button { text:"Rename"; onClicked:root.prompt("Rename page",root.page.name,name=>root.mutate(d=>{d.pages[root.pageIndex].name=name})) }
              Button { text:"Remove"; enabled:root.draft && root.draft.pages.length>1; opacity:enabled?1:0.4; onClicked:root.confirm("Remove this page?","Buttons that link to this page must be updated before saving.",()=>{var old=root.page.id;root.mutate(d=>{d.pages.splice(root.pageIndex,1);if(d.defaultPage===old)d.defaultPage=d.pages[0].id;if(d.fnPage===old)d.fnPage=null});root.pageIndex=0;root.widgetIndex=0}) }
            }
            RowLayout {
              Hint { text:"Default" }
              Ui.Dropdown { Layout.fillWidth:true; options:root.pageOptions; value:root.draft?root.draft.defaultPage:""; onChanged:value=>root.mutate(d=>{d.defaultPage=value}) }
              Hint { text:"Hold Fn" }
              Ui.Dropdown { Layout.fillWidth:true; options:[{value:"",label:"No alternate page"}].concat(root.pageOptions); value:root.draft?root.draft.fnPage || "":""; onChanged:value=>root.mutate(d=>{d.fnPage=value||null}) }
              Ui.Dropdown { options:[{value:"2170",label:"2170 · on-screen Esc"},{value:"2008",label:"2008 · physical Esc"}]; value:String(root.previewWidth); onChanged:value=>{root.previewWidth=Number(value);previewTimer.restart()} }
            }
            ColumnLayout {
                ...
                Repeater { model:root.boxes
                  Rectangle {
                    ...
                    MouseArea {
                      ...
                      onPressed:mouse=>{startX=mouse.x;root.widgetIndex=cell.index}
                      onReleased:mouse=>{
              ...
              contentWidth: availableWidth
                  ...
                  ColumnLayout { width:parent.width
                    Repeater { model:root.page?root.page.widgets:[]
                      Button { required property var modelData; required property int index; Layout.fillWidth:true; leftAlign:true; text:(index+1)+"  "+(modelData.label || modelData.type); selected:root.widgetIndex===index; onClicked:root.widgetIndex=index }
                    }
                  }
                }
                RowLayout {
              ...
              contentWidth: availableWidth
                ...
                ColumnLayout {
                  ...
                  RowLayout {
                    ...
                    Button { text:"Remove"; enabled:root.page && root.page.widgets.length>1; onClicked:{root.mutate(d=>d.pages[root.pageIndex].widgets.splice(root.widgetIndex,1));root.widgetIndex=Math.max(0,root.widgetIndex-1)} }
                  }
        ...
        ColumnLayout {
              ...
              Repeater { model:root.ruleDraft
                ColumnLayout {
                  ...
                  RowLayout {
                    ...
                    Ui.TextField { Layout.fillWidth:true; text:ruleRow.modelData.name; placeholderText:"Rule name"; onEditingFinished:root.editRule(ruleRow.index,"name",text) }
                    Ui.Dropdown { Layout.preferredWidth:200; options:root.presetOptions; value:ruleRow.modelData.preset; onChanged:value=>root.editRule(ruleRow.index,"preset",value) }
                    Button { text:"↑"; onClicked:root.moveRule(ruleRow.index,-1) }
                    Button { text:"↓"; onClicked:root.moveRule(ruleRow.index,1) }
                    Button { text:"Remove"; onClicked:{var r=root.copy(root.ruleDraft);r.splice(ruleRow.index,1);root.ruleDraft=r} }
                  }
                  RowLayout {
                    Ui.TextField { Layout.fillWidth:true; text:ruleRow.modelData.app; placeholderText:"Application pattern"; onEditingFinished:root.editRule(ruleRow.index,"app",text) }
                    Ui.TextField { Layout.fillWidth:true; text:ruleRow.modelData.title; placeholderText:"Window title contains (optional)"; onEditingFinished:root.editRule(ruleRow.index,"title",text) }
                  }
                  Line {}
                }
              }
          ...
          RowLayout {
            Button { text:"+ Add rule"; bordered:true; onClicked:{var r=root.copy(root.ruleDraft);r.push({id:"rule-"+Date.now(),name:"New rule",enabled:true,app:root.model.data.app || "*",title:"",preset:"everyday"});root.ruleDraft=r} }
            Item { Layout.fillWidth:true }
            Button { text:"Discard"; enabled:root.rulesDirty; onClicked:root.loadRules() }
            Button { text:"Save rules"; bordered:true; selected:root.rulesDirty; enabled:root.rulesDirty; onClicked:root.call("rules.save",{rules:root.ruleDraft},ok=>{if(ok){root.loadRules();root.notice="App rules saved"}},root.rulesRevision) }
          }
        }
        Controls.ScrollView {
              contentWidth: availableWidth
    ...
    Rectangle {
      anchors.fill:parent; visible:root.confirmAction!==null || root.promptVisible; color:Qt.rgba(0,0,0,0.6)
