...
Napi::Object TouchReader::Init(Napi::Env env, Napi::Object exports) {
  Napi::Function func = DefineClass(env, "TouchReader", {
    ...
    InstanceMethod("ranges", &TouchReader::Ranges),
...
// MarchyBar: query each digitizer's real ranges instead of assuming 0..32767.
Napi::Value TouchReader::Ranges(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  input_absinfo x{}, y{};
  if (ioctl(fd_, EVIOCGABS(ABS_MT_POSITION_X), &x) < 0 ||
      ioctl(fd_, EVIOCGABS(ABS_MT_POSITION_Y), &y) < 0 ||
      x.maximum <= x.minimum || y.maximum <= y.minimum) {
    Napi::Error::New(env, "Cannot read Touch Bar digitizer ranges").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  auto result = Napi::Object::New(env);
  result.Set("minX", x.minimum); result.Set("maxX", x.maximum);
  result.Set("minY", y.minimum); result.Set("maxY", y.maximum);
  return result;
}

Napi::Value TouchReader::Start(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (running_ || fd_ < 0) { Napi::Error::New(env, "Touch reader is already running or closed").ThrowAsJavaScriptException(); return env.Undefined(); }
